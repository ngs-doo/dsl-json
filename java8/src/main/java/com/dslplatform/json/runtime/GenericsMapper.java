...
//				if(raw.getGenericSuperclass() instanceof ParameterizedTypeImpl){
//					ParameterizedTypeImpl genericSuperclass = (ParameterizedTypeImpl) raw.getGenericSuperclass();
//					Type[] genericSuperclassActualTypeArguments = genericSuperclass.getActualTypeArguments();
//					for (int i = 0; i < c.getTypeParameters().length; i++) {
//						final TypeVariable tp = c.getTypeParameters()[i];
//						genericMappings.put(tp, genericSuperclassActualTypeArguments[i]);
//					}
//				}else {
...
//			}
    ...
    private void putAllIfAbsent(GenericsMapper mapper) {
        for (Map.Entry<GenericsMappingKey, Type> genericsMappingKeyTypeEntry : mapper.mappings.entrySet()) {
            mappings.putIfAbsent(genericsMappingKeyTypeEntry.getKey(), genericsMappingKeyTypeEntry.getValue());
        }
    }
