...
public class DecodePropertyInfo<T> {
	...
	static <T> DecodePropertyInfo<T>[] prepare(DecodePropertyInfo<T>[] initial) {
			...
			if (!hashes.add(ri.hash)) {
				for (int j = 0; j < decoders.length; j++) {
					...
					if (si.hash == ri.hash && !si.exactName) {
						decoders[j] = new DecodePropertyInfo<T>(ri.name, true, ri.mandatory, ~0, ri.index, ri.nonNull, ri.hash, ri.weakHash, ri.value, ri.nameBytes);
					}
				}
			}
			if (ri.mandatory) {
		...
		final HashMap<String, Integer> nameOrder = new HashMap<String, Integer>();
		for (int i = 0; i < decoders.length; i++) {
