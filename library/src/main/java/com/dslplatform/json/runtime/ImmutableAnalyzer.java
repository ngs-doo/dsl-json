...
public abstract class ImmutableAnalyzer {
	...
	private static String[] extractNames(Constructor<?> ctor) {
		try {
			return tryParanamerIfPresent(ctor);
	...
	private static <T> ImmutableDescription<T> analyze(final Type manifest, final Class<T> raw, final DslJson<?> json) {
		...
		final ArrayList<Constructor<?>> ctors = new ArrayList<Constructor<?>>();
		...
		final LinkedHashMap<String, JsonWriter.WriteObject> fields = new LinkedHashMap<String, JsonWriter.WriteObject>();
		final LinkedHashMap<String, JsonWriter.WriteObject> methods = new LinkedHashMap<String, JsonWriter.WriteObject>();
		...
		final LinkedHashMap<String, Field> matchingFields = new LinkedHashMap<String, Field>();
		for (final Field f : raw.getFields()) {
			if (isPublicFinalNonStatic(f.getModifiers())) {
				matchingFields.put(f.getName(), f);
			}
		}
		final LinkedHashMap<String, Method> matchingMethods = new LinkedHashMap<String, Method>();
		...
		if (names != null) {
			...
			if (matchingFields.size() == paramTypes.length) {
				List<Field> orderedFields = new ArrayList<Field>(matchingFields.values());
				for (int i = 0; i < paramTypes.length; i++) {
		...
		for (int i = 0; i < paramTypes.length; i++) {
			...
			readProps[i] = new DecodePropertyInfo<JsonReader.ReadObject>(names[i], false, false, i, false, new WriteCtor(json, concreteType, ctor));
		}
