...
public class Analysis {
	...
	private ConverterInfo validateConverter(TypeElement converter, Element target, String fullName) {
		...
		if (!converter.getModifiers().contains(Modifier.PUBLIC)) {
			...
			messager.printMessage(
					Diagnostic.Kind.ERROR,
					"Specified converter: '" + converter.getQualifiedName() + "' doesn't have a JSON_READER and/or a JSON_WRITER " + allowed + ". It must have both public static JSON_READER/JSON_WRITER " + allowed + " for conversion.",
					converter,
					getAnnotation(converter, converterType));
